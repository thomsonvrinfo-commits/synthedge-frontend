...
export async function me(): Promise<AuthUser | null> {
  try {
    const res = await apiClient.get<{ ok: boolean; user: AuthUser }>("/auth/me");
...
/**
 * The auth Worker's PATCH /auth/me only accepts { full_name } and returns
 * { ok: true } (no user body) — see workers/auth/src/handlers/tokens.ts.
 * Re-fetches via me() afterward so callers still get the updated user back.
 */
export async function updateMe(data: Partial<AuthUser>): Promise<AuthUser | null> {
  await apiClient.patch<{ ok: boolean }>("/auth/me", data);
  return me();
}
...
/**
 * The auth Worker emails a 6-digit code (not a magic-link token), and needs
 * the email address to look it up — see workers/auth/src/handlers/passwordReset.ts.
 */
export async function resetPassword({
  ...
  await apiClient.post<void>("/auth/reset-password", { email, resetCode, newPassword }, { auth: false });
}
