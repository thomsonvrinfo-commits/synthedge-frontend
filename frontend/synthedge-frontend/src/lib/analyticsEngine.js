/**
 * analyticsEngine.js — Central Analytics Engine (SynthEdge Launch Commander V2.1)
...
function getPL(trade) {
  return trade.pl ?? trade.profit_loss ?? trade.pnl ?? 0;
}

function getRR(trade) {
  return trade.rr ?? trade.risk_reward_ratio ?? 0;
}
...
export function computeAnalytics(rawTrades = []) {
  ...
  const totalPL =
    trades.reduce((sum,t)=>sum+getPL(t),0);



  const netRR =
    trades.reduce((sum,t)=>sum+getRR(t),0);



  const rrTrades =
    trades.filter(t=>getRR(t)!==undefined);
  ...
  const avgWinR =
    winTrades.length
    ?
    winTrades.reduce((s,t)=>s+getRR(t),0)
    /
    winTrades.length
    :
    0;
  ...
  const avgLossR =
    ...
    ?
    Math.abs(
      lossTrades.reduce((s,t)=>s+getRR(t),0)
      /
      lossTrades.length
    )
    :
  ...
  const avgExecution =
    execTrades.length
    ?
    execTrades.reduce((s,t)=>s+t.execution_rating,0)
    /
    execTrades.length
    :
    0;
  ...
  const growthRate =
    ...
    (
      ...
      /
      Math.max(Math.abs(equityCurve[0].equity),1)
    ) * 100
    :
    0;
  ...
  const emotionMap =
    groupStats(trades,"emotional_state");
